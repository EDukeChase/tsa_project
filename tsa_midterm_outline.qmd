...
I define the effective forecast horizon as$$h^* = \max\{h : \mathrm{CMAE}_{model}(h) < \mathrm{CMAE}_{\text{snaive}}(h)\},$$ where $\mathrm{CMAE}_{\text{snaive}}(h)$ is computed analogously for a seasonal naive baseline. If the SARIMA never beats the seasonally naive model, I'll denote that with $h* = 0$
\color{black}
...
\color{blue}
!!!BE SURE TO REWORK THIS SECTION TO DISTINGUISH IT FROM PART (A) MORE!!!
To get a good idea of the reliable forecast horizon, I'll use a train/test split to be able to see how my model performs on real world data. To determine how resilient SARIMA modeling is to shocks, I will actually have two different models: one that will be a pre-pandemic (PP) control model that uses the flu seasons before the 2017-18 season as training data and tests on the 2018-19 season, and then a full history (FH) model that uses the years before the 2023-24 season as training data and tests on the 2024-25 flu season. 
    
For both models, I'll calculate the RMSE for each week of the forecast and compare it to a seasonally naive baseline that just assumes any given week will be the same as the same week in the previous year (error margins for these point estimates will be computed based on prior year values for the same week). Wherever the SARIMA error becomes worse than the naive error, that's the limit of my effective forecasting window. I can then compare this window in the 2018-19 vs 2024-25 flu seasons and quantify how much the shock of COVID-19 affected model performance.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
\color{black}
