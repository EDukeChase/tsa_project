...
```{r}
#| label: seasonal-naive_model

# Build seasonally naive model
snaive_model <- snaive(train_ts, h = length(test_ts))
...
autoplot(snaive_model, include = 104) +
  ...
  labs(
    ...
    substitle = "Logic: Prediction = Observation from same week in previous year.",
    y = "Weighted ILI%",
    x = "Year"
  )
```
...
print(paste("nsdiffs (OCSB):", nsdiffs(ili_ts, test = "ocsb")))
print(paste("nsdiffs (SEAS):", nsdiffs(ili_ts, test = "seas")))
...
print(paste("ndiffs (KPSS):", ndiffs(ili_ts, test = "kpss")))
print(paste("ndiffs (ADF):",  ndiffs(ili_ts, test = "adf")))
...
adf_res <- adf.test(ili_ts)
kpss_res <- kpss.test(ili_ts, null = "Trend")
...
```{r}
#| label: sarima-grid-search

sarima_thorough <- auto.arima(train_ts, stepwise = FALSE, approximation = FALSE,
                              lambda = "auto", trace = TRUE)

print(sarima_thorough)
```
...
metrics_d0_D0 <- accuracy(forecast(sarima_d0_D0, h = 52), test_ts)
metrics_d0_D1 <- accuracy(forecast(sarima_d0_D1, h = 52), test_ts)
metrics_d1_D0 <- accuracy(forecast(sarima_d1_D0, h = 52), test_ts)
metrics_d1_D1 <- accuracy(forecast(sarima_d1_D1, h = 52), test_ts)
