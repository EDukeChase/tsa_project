...
```{r}
adf.test(ili_ts, alternative = c("stationary"))
kpss.test(ili_ts, null = c("Level"))
```
Both the Augmented Dickey-Fuller and KPSS tests indicate that the data are stationary, so that is nice to start off with.
