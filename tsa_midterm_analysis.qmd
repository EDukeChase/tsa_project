...
nav_cols <- scico::scico(6, palette = "navia")
```
...
#| label: model_metrics
#| 
metrics_tbl <- models_tbl |>
  mutate(
    # Extract AICc from fitted models
    AICc = map_dbl(fit, ~ .x$aicc),

    # Pull ARIMA orders (p, q, P, Q) from the arma vector
    arma_vec = map(fit, ~ .x$arma),
    p = map_int(arma_vec, 1L),
    q = map_int(arma_vec, 2L),
    P = map_int(arma_vec, 3L),
    Q = map_int(arma_vec, 4L),

    # In-sample accuracy
    train_acc  = map(fit, ~ forecast::accuracy(.x)["Training set", c("RMSE", "MAE")]),
    RMSE_train = map_dbl(train_acc, 1L),
    MAE_train  = map_dbl(train_acc, 2L),

    # Out-of-sample accuracy on test season (descriptive only)
    test_acc = map2(fit, dataset, ~ {
      s  <- get_series(.y)
      fc <- forecast::forecast(.x, h = length(s$test), biasadj = TRUE)
      forecast::accuracy(fc, s$test)["Test set", c("RMSE", "MAE")]
    }),
    RMSE_test = map_dbl(test_acc, 1L),
    MAE_test  = map_dbl(test_acc, 2L)
  ) |>
  select(
    tag, dataset, d, D, p, q, P, Q,
    AICc, RMSE_train, MAE_train, RMSE_test, MAE_test
  ) |>
